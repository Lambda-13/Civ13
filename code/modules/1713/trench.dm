		...
		if (src && istype(src, /turf/floor/trench))
			...
			if (istype(src, /turf/floor/trench/flooded))
				...
				for (var/turf/floor/TF in range(1, src))
					if (map && (istype(TF, /turf/floor/beach/water) || istype(TF, /turf/floor/trench/flooded)) && (map.civilizations || map.nomads || map.ID == MAP_COLONY || map.ID == MAP_FOUR_COLONIES || map.ID == MAP_PIONEERS || map.ID == MAP_BOHEMIA || map.ID == MAP_BOHEMIARU))
						flooded = TRUE
