...
/datum/job/civilian/policeofficer/equip(var/mob/living/human/H)
	...
	H.equip_to_slot_or_del(new /obj/item/stack/money/dollar/ten(H), slot_r_hand)
	H.add_note("Role", "Вы являетесь сотрудником полиции. Ваша цель - арестовать как можно больше нарушителй и задержать деньги и диски!")
	...
	H.add_note("Police Codes", "Как офицер, вы можете использовать полицейские коды для быстрого вещания. Он будет автоматически преобразован в обычный текст. Просто используйте префикс радио, за которым следует код, например, \";10-4\" для подтверждения.")
