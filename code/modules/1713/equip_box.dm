...
/obj/item/bisinessbox/attack_self(mob/living/user)
	...
	options["Banker/Rich"] = list(/obj/item/stack/money/dollar/onehundy,/obj/item/stack/money/dollar/onehundy,/obj/item/stack/money/dollar/onehundy,/obj/item/stack/money/dollar/onehundy,/obj/item/stack/money/dollar/onehundy,/obj/item/clothing/gloves/watch/goldwatch,/obj/item/weapon/storage/backpack/civbag)
	options["Metallurgist"] = list(/obj/item/stack/material/iron/twentyfive,/obj/item/stack/material/iron/twentyfive,/obj/item/stack/material/steel/twentyfive,/obj/item/stack/material/steel/twentyfive,/obj/item/weapon/storage/toolbox/mechanical,/obj/item/weapon/storage/backpack/civbag)
	var/choice = input(user,"What type of equipment?") as null|anything in options
	if(src && choice)
		var/list/things_to_spawn = options[choice]
		for(var/new_type in things_to_spawn)
			...
			if(istype(AM, /obj/item/weapon/key/civ/room1))
				to_chat(user, "Вы выбрали продавца домов. Дома для продажи находятся немного выше пожарных.")
			if(istype(AM, /obj/item/weapon/key/civ/gov))
				to_chat(user, "Вы выбрали работника Радиостанции.")
			if(istype(AM, /obj/item/clothing/gloves/watch/goldwatch))
				to_chat(user, "Вы выбрали банкира/богача.")
			if(istype(AM, /obj/item/weapon/storage/toolbox/mechanical))
				to_chat(user, "Вы продаёте или используете в своих делах металл и сталь, а так-же имеете ящик с инструментами.")
			if(istype(AM, /obj/item/weapon/storage/backpack/civbag))
				to_chat(user, "Рекомендую посмотреть карту что около вашего места появления - на ней отмечены точки работы.")
		qdel(src)