...
/client/proc/massmodify_variables(var/atom/O, var/var_name = "", var/method = FALSE)
	...
	for (var/p in forbidden_varedit_object_types)
		if ( istype(O,p) )
			usr << "<span class = 'red'>It is forbidden to edit this object's variables.</span>"
			return
	...
	message_admins("[key_name_admin(src)] mass modified [original_name]'s [variable] to [O.vars[variable]]", key_name_admin(src))
