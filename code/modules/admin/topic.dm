/datum/admins/Topic(href, href_list)
	...
	/*		var/rights = FALSE
		...
		switch(href_list["simplemake"])
			...
			if ("cow")			M.change_mob_type( /mob/living/simple_animal/cattle/cow , null, null, delmob )
			if ("bull")			M.change_mob_type( /mob/living/simple_animal/cattle/bull , null, null, delmob )
		...
		if (ismob(M))
			...
			if (!reason)
				...
				M << "<span class = 'userdanger'>Ты был кикнут с сервера по причине ([reason])</span>"
			log_admin("[key_name(usr)] kicked [key_name(M)].")
			message_admins("<span class = 'notice'>[key_name_admin(usr)] кикнул [key_name_admin(M)].</span>", key_name_admin(usr))
			//M.client = null
			qdel(M.client)
