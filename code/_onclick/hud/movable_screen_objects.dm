...
/obj/screen/movable/proc/encode_screen_X(var/X, var/mob/viewer)
	var/view = viewer.client ? viewer.client.view_num_x : WORLD_VIEW_NUM_X
	if(X > view+1)
		. = "EAST-[view*2 + 1-X]"
	else if(X < view+1)
		. = "WEST+[X-1]"
...
/obj/screen/movable/proc/encode_screen_Y(var/Y, var/mob/viewer)
	var/view = viewer.client ? viewer.client.view_num_y : WORLD_VIEW_NUM_Y
	if(Y > view+1)
		. = "NORTH-[view*2 + 1-Y]"
	else if(Y < viewer.client.view_num_y+1)
		. = "SOUTH+[Y-1]"
	else
		. = "CENTER"
