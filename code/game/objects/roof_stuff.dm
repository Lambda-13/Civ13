		...
		if (istype(W, /obj/item/flashlight/torch))
			...
			if (T.on)
				...
				if (prob(30))
					...
					message_admins("<span class = 'warning'>!!!</span> [user.name] ([user.ckey]) поджёг крышу \a [src] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>) <span class = 'warning'>!!!</span>")
					log_game("<span class = 'warning'>!!!</span> [user.name] ([user.ckey]) поджёг крышу \a [src] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>) <span class = 'warning'>!!!</span>")
