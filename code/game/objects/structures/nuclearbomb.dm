...
/obj/structure/nuclearbomb // Вообще это должна быть machinery а не структура
...
/obj/machinery/nuclearbomb/attackby(obj/item/weapon/O as obj, mob/user as mob)
	...
	if (src.extended)
		if (istype(O, /obj/item/weapon/disk/nuclear))
			...
			O.loc = src
			src.auth = O
			playsound(src.loc, 'sound/effects/nuke/nuke_activator2.ogg', 60, 0, -1)
			src.add_fingerprint(user)
			return
		...
		if (src.auth)
			...
			if (src.yes_code)
				...
				if (href_list["anchor"])
					...
					if(src.anchored)
						visible_message("\red With a steely snap, bolts slide out of [src] and anchor it to the flooring.")
						playsound(src.loc, 'sound/effects/nuke/nuke_activator.ogg', 60, 0, -1)
					else
...
/obj/machinery/nuclearbomb/proc/explode()
	...
	if(!src.lighthack)
		src.icon_state = "nuke2"
	playsound(src.loc, 'sound/effects/nuke/nuke_active.ogg', 60, 0, -1)
	if (ticker && ticker.mode)
		ticker.mode.explosion_in_progress = 1
		playsound(src,'sound/effects/nuke/nukee.ogg',100,0,5)
	sleep(5)
