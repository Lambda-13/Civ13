						...
						for (var/i in biomes)
							if (i == currcl)
								...
								for (var/k in seasons)
									if (season == k)
										count++
						if (count > 0 || (map.ID != MAP_NOMADS_CONTINENTAL && map.ID != MAP_NOMADS_PANGEA && map.ID != MAP_NOMADS_NEW_WORLD && map.ID != MAP_NOMADS_MEDITERRANEAN && map.ID != MAP_NOMADS_EUROPE && map.ID != MAP_NOMADS_FLY))
							stage += 1
					growth()
