		...
		if (istype(src, /turf/floor/dirt/ploughed) && istype(H) && is_plowed == TRUE)
			...
			if (istype(C, /obj/item/stack/farming/seeds/sapodilla))
				...
				if (C.amount>1)
					...
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/medical/advanced/herbs))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/herbs(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/celery))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/celery(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/liquorice))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/liquorice(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/juniper))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/juniper(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/sapote))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/sapote(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				...
				if (C.amount>1)
					...
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/beans))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/beans(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				...
				if (C.amount>1)
					...
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/barley))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/barley(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
				return
			else if (istype(C, /obj/item/stack/farming/seeds/oat))
				visible_message("[user] places the seeds in the ploughed field.")
				new/obj/structure/farming/plant/oat(src)
				if (C.amount>1)
					C.amount -= 1
				else
					qdel(C)
...
*/
	...
	if(prob(20) && (map.ID != MAP_NOMADS_PANGEA || map.ID != MAP_NOMADS_CONTINENTAL || map.ID != MAP_NOMADS_FLY || map.ID != MAP_NOMADS_NEW_WORLD && map.ID != MAP_NOMADS_MEDITERRANEAN && map.ID != MAP_GULAG13 && map.ID != MAP_RIVER_KWAI) && map.ordinal_age >=1)
	...
	else if(prob(20) && (map.ID == MAP_NOMADS_PANGEA || map.ID == MAP_NOMADS_CONTINENTAL || map.ID == MAP_NOMADS_FLY || map.ID == MAP_NOMADS_NEW_WORLD && map.ID == MAP_NOMADS_MEDITERRANEAN) && area_above && area_above.climate == "temperate" || area_above && area_above.climate == "semiarid" && map.ordinal_age >=1)
		var/obj/item/stack/material/marble/mineral = new/obj/item/stack/material/marble(src)
		mineral.amount = rand(8,12)
		if (istype(get_area(src), /area/caribbean/void/caves/special))
			mineral.amount *= 2
