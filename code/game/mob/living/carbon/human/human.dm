	...
	if (.)
		...
		if (client.status_tabs && statpanel("Персонаж"))
			...
			switch(original_job_title)
				...
				if ("Redmenian Civilian")
					stat("Industrial Research:","[map.civa_research[1]]/[civmax_research[1]]")
					stat("Military Research:","[map.civa_research[2]]/[civmax_research[2]]")
					stat("Health Research:","[map.civa_research[3]]/[civmax_research[3]]")
				if ("Blugoslavian Civilian")
					stat("Industrial Research:","[map.civa_research[1]]/[civmax_research[1]]")
					stat("Military Research:","[map.civa_research[2]]/[civmax_research[2]]")
					stat("Health Research:","[map.civa_research[3]]/[civmax_research[3]]")
				if ("Nomad")
	...
	if(href_list["interaction"])
		...
		if (href_list["interaction"] == "bow")
			...
			if(((Adjacent(P) && !istype(P.loc, /obj/structure/closet)) || (H.loc == P.loc)) && hashands)
				...
				playsound(loc, 'lambda/sanecman/sound/interactions/hug.ogg', 50, 1, -1)
			...
			if(((Adjacent(P) && !istype(P.loc, /obj/structure/closet)) || (H.loc == P.loc)) && hashands)
				...
				playsound(loc, 'lambda/sanecman/sound/interactions/slap.ogg', 50, 1, -1)
...
/mob/living/human/proc/has_penis()
	if(gender == MALE && potenzia > -1 && species.genitals && !mutilated_genitals)
		return 1
	else return 0

/mob/living/human/proc/mutilate_genitals()
