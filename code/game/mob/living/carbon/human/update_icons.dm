...
/mob/living/human/update_inv_head(var/update_icons=1)
	...
	if (head)
		...
		if (istype(head, /obj/item/clothing/head/custom_off_cap))
			...
			standing.overlays += band
			standing.overlays += cap
		else if (istype(head, /obj/item/clothing/head/custom_keffiyeh))
...
/mob/living/human/update_fire(var/update_icons=1)
	...
	if (drowning || water_overlay)
		...
		var/image/I = image("icon"='icons/ru/misc/beach.dmi', "icon_state"="[D.icon_state]_ov", "layer"=8)
		if (lying || prone)
			var/matrix/M = matrix()
			M.Scale(size_multiplier)
			M.Translate(0, 16*(size_multiplier-1))
...
/mob/living/human/proc/update_surgery(var/update_icons=1)
	...
	if (disease == TRUE)
		if (disease_type == "plague")
			...
			if (disease_progression >= 1 && disease_progression < 120)
				total.overlays += image(icon = 'icons/mob/human_races/masks/sickness.dmi', icon_state="rash1")
			else if (disease_progression >= 120)
				total.overlays += image(icon = 'icons/mob/human_races/masks/sickness.dmi', icon_state="rash2")
