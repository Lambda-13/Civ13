		...
		if (base_faction)
			...
			switch (original_job.base_type_flag())
				...
				if (RUSSIAN)
					...
					if (map.ID == MAP_YELTSIN || map.ID == MAP_GROZNY || map.ID == MAP_FACTORY || map.ID == MAP_FACTORY_AW || map.ID == MAP_PERVOMAISK_CROSSING)
						holder2.icon_state = "ru_basic"
				if (GERMAN)
				...
				if (CIVILIAN)
					...
					else if (map.ID == MAP_FACTORY || map.ID == MAP_FACTORY_AW || map.ID == MAP_PERVOMAISK_CROSSING)
