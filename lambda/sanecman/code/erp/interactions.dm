...
/mob/living/human/MouseDrop_T(mob/M as mob, mob/user as mob)
...
///mob/living/human/verb/interact(mob/M as mob)
//	set name = "Interact"
//	set category = "IC"
//
//	if (istype(M, /mob/living/human) && usr != M)
//		partner = M
...
/mob/living/human/proc/get_pleasure_amt(hole)
	switch (hole)
		if ("anal")
			return 7 - (potenzia/5)
		if ("anal-2")
...
/mob/living/human/make_interaction()

	var/mob/living/human/H = usr
	var/mob/living/human/P = H.partner
...
//				(hasvagina_p && (!P.mutilated_genitals))
...
//INTERACTIONS
/mob/living/human
	var/mob/living/human/partner
	var/mob/living/human/lastfucked
	var/lfhole
	var/potenzia = 10
	var/resistenza = 200
	var/lust = 0
...
/mob/living/human/proc/cum(mob/living/human/H as mob, mob/living/human/P as mob, var/hole = "floor")
	var/message = ""
	var/turf/T
	...
	if (H.gender == MALE)
		...
		playsound(loc, "lambda/sanecman/sound/interactions/final_m[rand(1, 5)].ogg", 70, 1, 0)
		...
		if (istype(P.loc, /obj/structure/closet))
			P.visible_message("<B>[H] [message].</B>")
		playsound(loc, "lambda/sanecman/sound/interactions/final_f[rand(1, 3)].ogg", 90, 1, 0)
...
mob/living/human/proc/fuck(mob/living/human/H as mob, mob/living/human/P as mob, var/hole)
	var/message = ""
	switch(hole)
		...
		if("fingering")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					...
					P.moan()
			playsound(loc, "lambda/sanecman/sound/interactions/champ_fingering.ogg", 50, 1, -1)
			H.do_fucking_animation(P)
		...
		if("blowjob")
			...
			if (P.stat != DEAD && P.stat != UNCONSCIOUS)
				...
				if (P.lust >= P.resistenza)
					P.cum(P, H, "mouth")
			playsound(loc, "lambda/sanecman/sound/interactions/bj[rand(1, 11)].ogg", 50, 1, -1)
			H.do_fucking_animation(P)
			if (prob(P.potenzia))
				H.halloss += 3
		...
		if("vaginal")
			...
			if (P.stat != DEAD)
				...
				if (H.potenzia > 20)
					P.halloss += H.potenzia * 0.25
				if (P.lust >= P.resistenza)
					P.cum(P, H)
				else
					P.moan()
			H.do_fucking_animation(P)
			playsound(loc, "lambda/sanecman/sound/interactions/bang[rand(1, 3)].ogg", 70, 1, -1)
		...
		if("oral")
			...
			H.do_fucking_animation(P)
			playsound(loc, "lambda/sanecman/sound/interactions/oral[rand(1, 2)].ogg", 70, 1, -1)
		...
		if ("Human")
			if (prob(H.lust / H.resistenza * 65))
				...
				if(!istype(loc, /obj/structure/closet))
					playsound(loc, "lambda/sanecman/sound/interactions/moan_[g][moan].ogg", 70, 1, 1)
				else if (g == "f")
					playsound(loc, "lambda/sanecman/sound/interactions/under_moan_f[rand(1, 4)].ogg", 70, 1, 1)
				lastmoan = moan


/mob/living/human/proc/handle_lust()
	lust -= 4
	if (lust <= 0)
		lust = 0
		lastfucked = null
...
/mob/living/human/proc/do_fucking_animation(mob/living/human/P)
	var/pixel_x_diff = 0
	var/pixel_y_diff = 0
...
/obj/item/weapon/enlarger/attack(mob/living/human/M, mob/living/human/user)
	if(istype(M, /mob/living/human) && (M.gender == MALE))
		M.potenzia = 30
		M << "<span class='warning'>Your penis extends!</span>"

	else if (istype(M, /mob/living/human) && M.gender == FEMALE)
...
/obj/item/weapon/dildo
	...
	icon = 'lambda/sanecman/icons/obj/items/dildo.dmi'
	icon_state = "dildo"
	item_state = "c_tube"
	throwforce = 0
	force = 10
...
/obj/item/weapon/dildo/attack(mob/living/human/M, mob/living/human/user)
	...
	if(istype(M, /mob/living/human) && user.zone_sel.selecting == "groin" && M.is_nude())
		if (hole == "vagina" && hasvagina)
			if (user == M)
				message = pick("заталкивает в себя [name]", "погружает [name] в свое лоно")
			else
				message = pick("заталкивает в [M] [name]", "погружает [name] в лоно [M]")
			...
			playsound(loc, "lambda/sanecman/sound/interactions/bang[rand(4, 6)].ogg", 70, 1, -1)
